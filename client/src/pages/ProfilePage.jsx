  ...
  return (
    <div className="min-h-screen bg-slate-50 relative pb-20">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 pt-12 md:pt-20">
        ...
        {/* --- Content Tabs --- */}
        <div className="flex items-center gap-6 border-b border-slate-200 mb-8 px-4">
             {['issues', 'activity'].map(tab => (
                 <button 
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`pb-4 text-sm font-bold capitalize transition-colors relative ${activeTab === tab ? 'text-slate-900' : 'text-slate-400 hover:text-slate-600'}`}
                 >
                    {tab === 'issues' ? 'My Reports' : 'Recent Activity'}
